...
class ordered extends Smarty
{
    function loadmore()
    {
        global $db;
         ...
         $sql="select  sim1,sim2, giaban, mang FROM sim where sim2 LIKE '%".substr(trim($_GET['sosim']),-5,5)."' AND sim2!='".trim($_GET['sosim'])."' limit 30";
        $cache_key=md5($sql);
        $cache_path="seo";
        $data="";
        $data=cache($cache,null,"+1 day","cache/".$cache_path);
                      ...
                      foreach ($data as $row) {
                            ...
                            $li .='<li><a class="pull-left" href="/'.$row['sim2'].'.html"><strong class="sim">'.$row['sim1'].'</strong></a><span class="pull-right">'.number_format($row['giaban']*1000000).' VNĐ</span></li>';                      }
