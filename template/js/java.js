...
$(".error").remove();
  ...
  if(phone.val()=='')
  {
    phone.focus().after('<span class="error">Bạn chưa nhập điện thoại!</span>');
     return false;
    
  }
  if(city.val()=='')
  {
    city.focus().after('<span class="error">Bạn chưa chọn tỉnh thành!</span>');
    return false;
  }
  /**
  if(qh.val()=='')
  {
    qh.focus().after('<span class="error">Bạn chưa chọn quận huyện!</span>');
     return false;
  }
  
  if(px.val()=='')
  {
    px.focus().after('<span class="error">Bạn chưa chọn phường xã!</span>');
     return false;
  }
  **/
  if(address.val()=='')
...
{
    $("body").appendTo("<div class=\"msgshow alert alert-success\">"+msg+"</div>");
}
$.fn.toggleCheck=function(){
       if(this.tagName==='INPUT'){
           $(this).prop('checked', !($(this).is(':checked')));
...
$(".pricetoogle").click(function(){
    ...
    $.post(thisdomain+'/index.php?ajax=toggle.price').done(function(){
        window.location.reload();
    })

})
$("#sim").popover({html: true,trigger: 'focus'});
...
$("input[name='simtype']").click(function(){
        $("body").addClass("loading");
        $.post(thisdomain+'/index.php?ajax=toggle.simtype',{'simtype':$(this).val()}).done(function(){
        window.location.reload();
    })
})
 ...
 $('.togglecheck').click(function(){
    $('input[name="check\[\]"]').each(function(){ this.checked = ! this.checked });
 })
...
$(".checktoggle").click(function(){
    ...
    $(document).on('click', '.delcart',function(){
        ...
        $.ajax({
            url: thisdomain+'/index.php?ajax=ordered.delmycart&id='+sim,
            cache: false,
    ...
    $(".addcart").click(function(){
        ...
        $("#myModal").modal('show');
          ...
          $.ajax({
            url: thisdomain+'/index.php?ajax=ordered.mycart&add='+sim+'&gia='+gia,
            cache: false,
            success:function(data)
            {
    ...
    $(".modal-content").on('submit','.ajax-form',function(e){
        
        var $action = $(this).attr('action');
        
        var $form = $(this);
        $(".ajax-form").find("input").removeClass('has-error has-feedback')
        ...
        $.ajax({
            ...
            success:function(json)
            {
               ...
               $(".ajax-form").find("button[type='submit']").attr('disabled',false)
            loadconplex()
            
               
               if(json['ok'])
               {
                $(".ajax-form").before("<div class=\"alert alert-success\">"+json['ok']+"</div>");
                ...
                $.each(json,function(key,val){
					...
					if(json[key])
					{
					   ...
					   $input=$(".ajax-form input[name='"+key+"'], .ajax-form textarea[name='"+key+"'],  .ajax-form select[name='"+key+"']");
                       $input.parent().addClass('has-error has-feedback');
                       $input.after("<span class='error'>"+json[key]+"</span>")
    ...
    $(".lock").click(function(){
        ...
        $.post('index.php?ajax=users.lock',{'id' : id}).done(function(data){
    ...
    $(".unactive").click(function(){
        ...
        $.post('index.php?ajax=users.active',{'id' : id}).done(function(data){
...
w=img.attr('width');
h=img.attr('height');
if(!w)w=img.css('width');
if(!h)h=img.css('height');
         ...
         if($(window).width() < 1024)
{
             ...
             nav=$(".navbar-left-fix").html();
             
              nav2=$("#menutop").html();
             var strVar="";
strVar += "<nav id=\"c-menu--slide-left\" class=\"navbar-default c-menu c-menu--slide-left\">";
...
strVar += "<\/nav>";
             ...
             $('body').append(strVar);
                var strVar="";
strVar += "<nav id=\"c-menu--slide-right\" class=\"navbar-default c-menu c-menu--slide-right\">";
...
strVar += "<\/nav>";
            ...
            $(".mega-menu-column li").css({'list-style':'none', 'height': '24px'});
              $("#c-menu--slide-right li").css({'list-style':'none', 'height': '24px'});
            ...
            /* 
             $("#menuleft").addClass('c-menu c-menu--slide-left')
              $("#menuleft").append($("<button class=\"c-menu__close\">áº¨n menu <span class=\"glyphicon glyphicon-arrow-right\"></span></button>"));
...
*/
  ...
  var slideLeftBtn = document.querySelector('.leftmenu');
